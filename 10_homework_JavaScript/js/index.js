...
console.log(
	`Его зовут ${firstName} ${lastName}. Он являюсь ${isStudent ? "учеником" : "не учеником"} курса.`,
);
...
console.log(
	`Студенту — ${age}лет, на же дворе — ${currentYear}. Следовательно, год рождения студента — ${birthYear}`,
);
...
console.log(
	`result переменная или ${result} является строковой, тип string. (let result: string)`,
);
